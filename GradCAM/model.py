...
from keras.layers import Dense, Activation, Conv2D, BatchNormalization, Dropout, MaxPool2D, Flatten, regularizers

def build_model():
    model = Sequential([
        Conv2D(64, kernel_size=5, strides=1, padding='same', input_shape=INPUT_SHAPE + (1,)),
...
def train(k_fold):
    ...
    checkpoint = keras.callbacks.ModelCheckpoint(path, monitor='val_loss', verbose=1,save_best_only=True, save_weights_only=True, mode='auto', period=1)
